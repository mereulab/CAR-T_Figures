...
The main objective of this notebook is to add the TCR information of alpha and beta chains to our CAR+ and CARneg T cells. To do so, we followed scRepertoire package that takes filter contig outputs from the 10x Genomics Cell Ranger pipeline and processes that data to assign clonotypes based on the TCR chains.
...
output_dir <- "../analysis/01_TCR_analysis/combined_files/"

# Loop through the combinations and perform the combination and saving
for (combination in combinations) {
  contigs <- lapply(combination$contigs, get)
  combined <- combineTCR(contigs, samples = combination$contigs, cells = "T-AB")
